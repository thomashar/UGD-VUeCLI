<template>
    <v-main class="list">
        ...
        <v-card>
            ...
            <v-data-table :headers="headers" :items="todos" :search="search">
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    ...
                    <input type="checkbox" v-model="item.hapus" 
                        style="margin-left: 100px;" @change="checked(item)">
                </template>
            </v-data-table>
        </v-card>
        ...
        <v-card v-if="checkedTodo.length">
            <v-card-title>Delete Multiple</v-card-title>
            <v-card-text>
                <ul v-for="(todo, index) in checkedTodo" :key="index">
                    <li>
                       {{todo.task}} 
                    </li>
                </ul>
            </v-card-text>
            <v-btn color="red darken-1" @click="deleteAll">
                Hapus Semua
            </v-btn>
        </v-card>
    </v-main>
...
export default {
    ...
    methods: {
        ...
        checked(item){
            if(item.hapus){
                this.checkedTodo.push(item);
            }else{
                let position = this.checkedTodo.findIndex(el=>el.task === item.task);
                this.checkedTodo.splice(position, 1);
            }
        },
        deleteAll(){
            this.todos = this.todos.filter(el=>!this.checkedTodo.includes(el));
            this.checkedTodo = [];
        }
    },
};
